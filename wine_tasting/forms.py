...
class WineTastingForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['wine_tasting_product', 'contact_number',
                  'special_requirements', 'selected_date', 'number_of_people']
        labels = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['wine_tasting_product'].choices = [
            ('', '-Experiences-')
            ] + [
            (product.id, product.product_name)
            for product in WineTastingProduct.objects.all()
        ]
    ...
    def clean_wine_tasting_product(self):
        ...
        try:
            ...
            raise forms.ValidationError(
                "Invalid wine tasting product selected."
                )
        return product
